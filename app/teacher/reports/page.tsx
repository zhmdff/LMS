    ...
    <div className="flex flex-col gap-6 text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border-border shadow-sm bg-white p-6 flex flex-col gap-1 text-left relative overflow-hidden group">
          ...
          <p className="text-3xl font-black text-[var(--color-primary)]">82.4</p>
        ...
        <Card className="border-border shadow-sm bg-white p-6 flex flex-col gap-1 text-left relative overflow-hidden group">
          ...
          <p className="text-3xl font-black text-blue-600">91.8%</p>
        ...
        <Card className="border-border shadow-sm bg-white p-6 flex flex-col gap-1 text-left relative overflow-hidden group">
          ...
          <p className="text-3xl font-black text-red-600">14 Students</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  {performanceStats.map((stat, idx) => (
                    <tr key={idx} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <span className={cn("px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border", parseInt(stat.failRate) > 10 ? "bg-red-50 text-red-700 border-red-100" : "bg-green-50 text-green-700 border-green-100")}>{stat.failRate}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="space-y-6">
          <Card className="border-border shadow-sm bg-[var(--color-primary)] text-white p-8 text-center flex flex-col items-center">
            ...
            <p className="text-xs text-slate-400 mt-2 font-medium leading-relaxed mb-8">Visualize the grade distribution across all your student groups to identify bell curve anomalies.</p>
            <Button className="w-full h-12 bg-white text-slate-900 font-black uppercase text-[11px] tracking-widest hover:bg-slate-50 shadow-xl border-none">Generate Visual Chart</Button>
          </Card>
              ...
              {[
                ...
                <div key={idx} className="space-y-2">
                  ...
                  <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                    <div className={cn("h-full rounded-full", idx === 0 ? "bg-emerald-500" : idx === 1 ? "bg-blue-500" : "bg-red-500")} style={{ width: `${(item.value / 220) * 100}%` }}></div>
                  </div>
                </div>
              ))}
            </CardContent>
