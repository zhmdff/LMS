                        ...
                        {weekDays.map(day => {
                           ...
                           return (
                              <div key={`${day}-${slot}`} className="p-2 border-l border-slate-100 first:border-l-0 min-h-[110px] hover:bg-slate-50/50 transition-colors">
                                 {classItem && (
                                    <div className={cn(
                                       "h-full rounded-2xl p-4 text-left border shadow-sm transition-all hover:shadow-md cursor-pointer",
                                       classItem.type === 'Lecture' ? "bg-white border-indigo-200" :
                                       classItem.type === 'Lab' ? "bg-white border-emerald-200" :
                                       "bg-white border-amber-200"
                                    )}>
                                       <div className="flex justify-between items-start mb-2">
                                          <div className={cn(
                                             "size-8 rounded-xl flex items-center justify-center",
                                             classItem.type === 'Lecture' ? "bg-indigo-50 text-indigo-600" :
                                             classItem.type === 'Lab' ? "bg-emerald-50 text-emerald-600" :
                                             "bg-amber-50 text-amber-600"
                                          )}>
                                             <BookOpen className="h-4 w-4" />
                                          </div>
                                          <div className="flex items-center gap-1 text-[9px] font-black uppercase text-slate-400 tracking-tighter">
