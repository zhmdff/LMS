    ...
    <div className="flex flex-col gap-6 text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="border-border shadow-sm bg-white p-6 flex flex-col gap-1 text-left">
           ...
           <p className="text-3xl font-black text-slate-900 tracking-tighter">92.4%</p>
        ...
        <Card className="border-border shadow-sm bg-white p-6 flex flex-col gap-1 text-left border-l-4 border-l-red-500">
           ...
           <p className="text-3xl font-black text-slate-900 tracking-tighter">8 Hours</p>
        ...
        <Card className="border-border shadow-sm bg-white p-6 flex flex-col gap-1 text-left">
           ...
           <p className="text-3xl font-black text-slate-900 tracking-tighter">4 Hours</p>
        ...
        <Card className="border-border shadow-sm bg-[var(--color-primary)] p-6 flex flex-col gap-1 text-left text-white">
           ...
           <p className="text-2xl font-black tracking-tighter uppercase">Academic Clear</p>
                           ...
                           <tr key={log.id} className="hover:bg-slate-50 transition-colors group">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                 <div className={cn(
                                    ...
                                    log.status === 'Absent' ? "bg-red-100 text-red-700" : "bg-blue-100 text-blue-700"
                                 )}>
                                    {log.status === 'Present' ? <CheckCircle2 className="h-3 w-3" /> : 
                                     log.status === 'Absent' ? <XCircle className="h-3 w-3" /> : <Clock className="h-3 w-3" />}
                                    {log.status}
                                 </div>
                              </td>
                           </tr>
                     ...
                     <div key={idx} className="space-y-2">
                        <div className="flex justify-between items-end">
                           <div>
                              ...
                              <p className="text-[10px] font-bold text-slate-400 mt-0.5">{sub.current} / {sub.limit} Hours Absent</p>
                           </div>
                           <span className={cn(
                              "text-xs font-black",
                              parseInt(sub.rate) > 90 ? "text-green-600" : "text-amber-500"
                           )}>{sub.rate}</span>
                        </div>
                        <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                           <div className={cn(
                              "h-full rounded-full transition-all duration-500",
                              parseInt(sub.rate) > 90 ? "bg-green-500" : "bg-amber-500"
                           )} style={{ width: sub.rate }}></div>
                        </div>
                     </div>
                  ))}
               </CardContent>
            ...
            <Card className="border-border shadow-sm bg-red-50 border-red-100 p-6 flex gap-4 text-left">
               ...
               <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-red-700 font-medium leading-relaxed">
                     Students exceeding 25% (18 hours) absence in any subject will be automatically banned from final exams.
                  </p>
               </div>
            </Card>
         </div>
