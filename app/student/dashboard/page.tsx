  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <DashboardCard title="Current GPA" value="3.84" icon={GraduationCap} />
        <DashboardCard title="Attendance" value="92%" icon={Calendar} trend={{ value: 2, isUp: false }} />
        <DashboardCard title="Absence Limit" value="4/6" icon={AlertTriangle} color="red" description="Risk in Software Engineering" />
        <DashboardCard title="Total Credits" value="30" icon={BookOpen} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div key={idx} className="p-6 flex items-center justify-between hover:bg-slate-50 transition-colors">
                  <div className="text-left flex-1">
                    <div className="flex items-center gap-3">
                      ...
                      {item.status === "At Risk" && <span className="bg-red-100 text-red-600 text-[10px] font-black uppercase px-2 py-0.5 rounded-full">At Risk</span>}
                    </div>
                    <p className="text-sm text-slate-500 font-medium">
                      {item.teacher} • Attendance: {item.attendance}
                    </p>
        ...
        <Card className="border-border shadow-sm">
          ...
          <CardContent className="p-4 space-y-4">
            ...
            <Button className="w-full h-11 bg-[var(--color-primary)] hover:bg-[var(--color-primary)]/90 text-white font-bold tracking-tight">Open Full Schedule</Button>
          </CardContent>
        </Card>
      </div>
    </div>
