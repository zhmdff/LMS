    ...
    <div className="flex flex-col gap-6 text-left">
      ...
      <Card className="p-4 flex flex-col md:flex-row gap-4 items-center justify-between border-border shadow-sm bg-white">
        <div className="relative w-full md:w-96">
          ...
          <input 
            type="text" 
            placeholder="Search groups by code or specialty..." 
               ...
               <tbody className="divide-y divide-border bg-white">
                  {groups.map((group) => (
                     <tr key={group.id} className="hover:bg-slate-50 transition-colors group">
                        <td className="px-6 py-4 whitespace-nowrap text-lg font-black text-[var(--color-primary)] tracking-tighter">{group.name}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                           <div className="flex items-center gap-2">
                             <div className={cn(
                               "size-6 rounded-full flex items-center justify-center text-[8px] font-black text-white",
                               group.tutor === 'Unassigned' ? "bg-slate-300" : "bg-emerald-500"
                             )}>
                                {group.tutor === 'Unassigned' ? '?' : group.tutor.split(' ').map(n => n[0]).join('')}
                             </div>
                             <span className={cn(
                                "text-xs font-bold",
                                group.tutor === 'Unassigned' ? "text-slate-400" : "text-slate-700"
                             )}>{group.tutor}</span>
                           </div>
                        </td>
