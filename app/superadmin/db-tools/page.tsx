    ...
    <div className="flex flex-col gap-6 text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="border-border shadow-sm bg-white p-6 flex flex-col gap-1 text-left relative overflow-hidden group">
          ...
          <p className="text-3xl font-black text-slate-900">4ms</p>
        ...
        <Card className="border-border shadow-sm bg-[var(--color-primary)] p-6 flex flex-col gap-1 text-left relative overflow-hidden group text-white">
          ...
          <p className="text-3xl font-black uppercase">Postgres</p>
          ...
          <CardHeader className="bg-slate-50/50 border-b border-border p-4 px-6 flex flex-row items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" className="h-8 bg-white text-[10px] font-black uppercase">
                <RefreshCw className="h-3 w-3 mr-2" /> SCAN
              </Button>
            </div>
          </CardHeader>
                ...
                <tbody className="divide-y divide-border bg-white">
                  {migrations.map((m) => (
                    <tr key={m.id} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={cn("px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest", m.status === "Success" ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600")}>{m.status}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="space-y-6">
          ...
          <Card className="border-border shadow-sm border-red-100 bg-red-50">
            <CardContent className="p-6 flex flex-col gap-4">
              ...
              <p className="text-xs text-red-700 font-medium leading-relaxed">Defragment indices and reclaim storage space from deleted records. This process might cause temporary high latency.</p>
      ...
      <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl flex gap-3 text-left">
        ...
        <p className="text-[11px] text-slate-400 font-bold leading-relaxed uppercase tracking-tight">Safety Warning: Any DDL instructions executed through this console bypass routine application-level validation. Ensure a snapshot is taken before manipulation.</p>
      </div>
    </div>
  );
}
